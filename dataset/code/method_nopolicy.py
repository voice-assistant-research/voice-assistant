...
def getdescription():
	...
	for row in reader:
		name=row[0].replace('/','')
		if (name in policy)==False:
			description[name]=row[1]
	csvfile.close()
	del description['skillname']
	return description
...
def compare(des):
	try:
		...
		dessents=removeyou(dessents)
		desphrase=getphrase(dessents)
		desphrase=removeunrelatedphrase(desphrase,threshold)
	except:
		return -2
	return desphrase
...
if __name__ == '__main__':
	...
	for i in description:
		...
		try:
			des=description[i]
			result[i]=compare(des,threshold)
			print(i,result[i])
			f=open('result_nopolicy.txt','a')
			a=f.write(i)
			a=f.write('\t')
			if result[i]==[]:
				a=f.write('0')
